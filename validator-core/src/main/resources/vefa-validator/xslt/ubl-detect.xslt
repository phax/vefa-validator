    ...
    <xsl:variable name="content">
      <array>
        <xsl:if test="cbc:CustomizationID[1] and cbc:ProfileID[1]">
          <string>
            <xsl:value-of select="concat(cbc:ProfileID[1]/normalize-space(), '#', cbc:CustomizationID[1]/normalize-space())"/>
          </string>
        </xsl:if>
        ...
        <xsl:if test="cbc:CustomizationID[1] and cbc:ProfileID[1]">
          <string>
            <xsl:value-of select="concat(local-name(), '::', cbc:ProfileID[1]/normalize-space(), '#', cbc:CustomizationID[1]/normalize-space())"/>
          </string>
        </xsl:if>
        <xsl:if test="cbc:CustomizationID[1]">
          <string>
