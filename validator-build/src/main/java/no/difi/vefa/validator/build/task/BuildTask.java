...
public class BuildTask
{
  ...
  public void build (final Build build) throws IOException, JAXBException
  {
        ...
        try
        {
          ...
          for (final ConfigurationType configuration : config.getConfiguration ())
          {
            ...
            configuration.setBuild (configuration.getBuild () != null ? configuration.getBuild () : build.getSetting (
                                                                                                                      "build"));
            configuration.setWeight (Long.valueOf (configuration.getWeight () != 0 ? configuration.getWeight () : Long
                                                                                                                      .parseLong (build.getSetting ("weight"))));
    ...
    AsicArchiver.archive (build.getTargetFolder ()
                               .resolve (build.getSetting ("name") + "-" + build.getSetting ("build") + ".asice"),
                          contentsPath);
